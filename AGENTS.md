...
- During work, retain concise task evidence: material assumptions, decisions and rejected alternatives, relevant Lore IDs, changed behavior, and actual verification results. For material decisions likely to guide later work, retain their rationale, supporting evidence, scope, relevant assumptions, and identified reconsideration conditions. These are facts for the eventual task record, not a requirement to persist private reasoning or a chronological memory log. Do not record secrets or sensitive data.
...
| Forming an implementation plan; editing or reviewing code, tests, configuration, or documentation; or reviewing architecture | [Development](.lore-coding/instructions/development.md) |
