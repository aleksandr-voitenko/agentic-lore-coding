...
- The bundle version is recorded only in this file. Install this file and the complete `.lore-coding/` directory from the same Git revision; modules do not carry individual version markers. If a required file is missing or unreadable, report the exact path and stop the dependent operation. Do not silently fall back to a guessed procedure.
