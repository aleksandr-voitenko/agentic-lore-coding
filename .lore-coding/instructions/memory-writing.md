...
`README.md` explains public project behavior, setup, development, testing, and deployment for humans. `MEMORY.md` gives agents and maintainers compact durable context before detailed task history. Do not make README an agent scratchpad or memory a chronological task log; commits already record tasks and their relationships.
...
Prefer scoped memory over one large root file. Child memory refines rather than contradicts parent memory. Investigate conflicts with code, tests, or recent history and update stale summaries using verified current evidence.

## Content and size

Keep memory compact. Suggested ranges are roughly 150–250 lines for root memory and 80–150 for a folder, not minimum lengths or targets to pad. Smaller repositories need less. Split or compact when a file grows beyond a useful local summary.
...
Before a task is finalized, review whether the README and relevant memory need changes. No-change reviews do not require a commit-message entry unless the user specifically requested them.

Update README for changes to public behavior, setup/build/run/test commands, deployment, environment variables, persistent storage, or project description. Keep it understandable without knowledge of the internal Lore workflow; avoid long internal file maps unless useful to contributors.
