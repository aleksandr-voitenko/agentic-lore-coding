...
For material decision summaries, retain the rationale, scope, relevant assumptions, and identified reconsideration conditions needed to understand applicability. Do not turn a conditional design choice into an unconditional rule. Compaction must preserve qualifications that affect when a decision applies. A future condition is not a verified claim that it has occurred.

Do not invent or require a future Lore ID or commit before review; use available source anchors and retained task evidence when the new task record does not yet exist. Add its task pointer during authorized finalization when useful. Do not create a commit just to supply a memory pointer.
