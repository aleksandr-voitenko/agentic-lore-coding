...
For material decisions likely to guide future work, retain the chosen approach, its rationale and supporting evidence, and the scope or assumptions on which it depends. Capture this evidence when the decision is made or revised, not only during finalization. Do not reconstruct unsupported rationale from the completed implementation. This is not a mandatory template for routine local choices.

Distinguish explicit requirements, design choices, and working assumptions. Record meaningful reconsideration conditions identified during the task as observable changes in requirements, evidence, dependencies, or operating conditions. Do not invent thresholds, deadlines, or future requirements to fill a template. A condition prompts review; it does not authorize reversing the decision or changing task scope, explicit requirements, or safety constraints. The absence of a recorded condition does not make a decision permanent or safe to discard. Apply the existing discovery and assumption rules when evidence or intent is missing.
