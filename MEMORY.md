...
Verification distinguishes expected validation from diagnostic reruns under changed conditions. A passing isolated or single-worker run does not resolve a failing normal check; unchanged files alone do not establish that a failure is unrelated. Short reports retain direct evidence and unresolved required-check status. Source: `.lore-coding/instructions/verification.md`.

`node --test tests/*.test.mjs` also covers these selected rerun and reporting safeguards in `tests/validation-reruns.test.mjs`. `tests/validation-rerun-scenarios.md` is a separate optional evaluation fixture; the existing Classic Games prompts are unchanged.
